...
- **PII never leaks to the anon key.** Supabase auto-grants `SELECT` to `anon` on new `public` views, and owner-run views bypass RLS. Any analyst view touching PII (e.g. `latest_submissions`, `alloc_long`, `alloc_canonical`) MUST be `create ... with (security_invoker = on)` AND `revoke all ... from anon, authenticated`. **No view is anon-readable now:** `public_allocations` (allocation counts, no PII) was the one exception, but Supabase's advisor flags every SECURITY DEFINER view as critical, so it's been locked down the same way (`security_invoker = on` + revoked from anon). Re-expose a public aggregate only via a deliberately-reviewed mechanism (a materialized view or a `security definer` function), never by reverting this.
